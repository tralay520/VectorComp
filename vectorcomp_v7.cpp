...
float KVVectorcompV7::compute_cosine_similarity(const float* a, const float* b) const {
...
int KVVectorcompV7::find_best_ltm_match(const float* k_new, float& out_sim) const {
    int best_idx = -1;
    out_sim = -1.0f;
...
void KVVectorcompV7::update_centroid(int idx, const float* k_new, const float* v_new) {
...
int KVVectorcompV7::get_evictable_ltm_slot() const {
    for (int i = 0; i < max_cb_slots; ++i) {
...
// RoPE applies: [x0, x1] -> [x0*cos(t) - x1*sin(t), x0*sin(t) + x1*cos(t)]
// Inverse: [y0, y1] -> [y0*cos(t) + y1*sin(t), -y0*sin(t) + y1*cos(t)]
void KVVectorcompV7::derotate_k(const float* k_rotated, int position, float* k_content) const {
    if (!rope_enabled || rope_inv_freq.empty()) {
        std::memcpy(k_content, k_rotated, static_cast<size_t>(head_dim) * sizeof(float));
        return;
    }

    for (int i = 0; i < head_dim / 2; ++i) {
        float freq = rope_inv_freq[i];
        float theta = static_cast<float>(position) * freq;
        float c = std::cos(theta);
        float s = std::sin(theta);
        float y0 = k_rotated[2 * i];
        float y1 = k_rotated[2 * i + 1];
        // Inverse rotation
        k_content[2 * i]     = y0 * c + y1 * s;
        k_content[2 * i + 1] = -y0 * s + y1 * c;
    }
}

uint32_t KVVectorcompV7::encode_shim(const float* k_new, const float* v_new) {
...
// V7: Encode with RoPE awareness — strips rotation before clustering
uint32_t KVVectorcompV7::encode_shim_rope(const float* k_rotated, const float* v_new, int position) {
    std::lock_guard<std::mutex> lock(mtx);

    global_step++;

    // Derotate K to get content-only representation for clustering
    std::vector<float> k_content(head_dim);
    derotate_k(k_rotated, position, k_content.data());

    float best_sim = 0.0f;
    int best_idx = find_best_ltm_match(k_content.data(), best_sim);

    if (best_idx != -1 && best_sim >= high_loose) {
        if (best_sim < high_strict) update_centroid(best_idx, k_content.data(), v_new);
        cb_metadata[best_idx].usage_count++;
        cb_metadata[best_idx].last_step = global_step;
        return static_cast<uint32_t>(best_idx);
    }
    ...
    if (best_idx != -1 && best_sim >= medium_thresh) {
        auto raw_offset = static_cast<size_t>(raw_ptr) * head_dim;
        std::memcpy(&k_raw_buffer[raw_offset], k_content.data(), static_cast<size_t>(head_dim) * sizeof(float));
        std::memcpy(&v_raw_buffer[raw_offset], v_new, static_cast<size_t>(head_dim) * sizeof(float));
        uint32_t id = RAW_FLAG | static_cast<uint32_t>(raw_total_count & 0x7FFFFFFF);
        raw_ptr = (raw_ptr + 1) % static_cast<uint64_t>(max_raw_size);
        raw_total_count++;
    ...
    int slot = get_evictable_ltm_slot();
    auto slot_offset = static_cast<size_t>(slot) * head_dim;
    std::memcpy(&k_codebook[slot_offset], k_content.data(), static_cast<size_t>(head_dim) * sizeof(float));
    std::memcpy(&v_codebook[slot_offset], v_new, static_cast<size_t>(head_dim) * sizeof(float));
    cb_metadata[slot].is_active = true;
    cb_metadata[slot].usage_count = 1;
    cb_metadata[slot].last_step = global_step;
...
void KVVectorcompV7::decode_shim(const uint32_t* ids, int seq_len, float* out_k, float* out_v) {
    std::lock_guard<std::mutex> lock(mtx);
...
size_t KVVectorcompV7::get_active_cb_count() const {
    size_t count = 0;
